...
function calculate() {
    ...
    let futureValue = calculateFutureValue(initialAmount, monthlyDeposit, annualInterestRate, years);
    document.getElementById("result").innerHTML = "Odhadovaná budoucí hodnota je: " + futureValue;
}

function calculateFutureValue(initialAmount, monthlyDeposit, annualInterestRate, years) {
    let months = years * 12;
    let monthlyInterestRate = annualInterestRate / 12 / 100;
    let futureValue = initialAmount * Math.pow((1 + monthlyInterestRate), months) + monthlyDeposit * (((Math.pow((1 + monthlyInterestRate), months)) - 1) / monthlyInterestRate);
